...
func _on_bp_small_door_body_entered(body: Node3D) -> void:
	...
	if body.is_in_group("player") and not RoomGenerated:
		...
		# 10% chance to pre-generate the next room in advance and attach it to this new room
		var rng = RandomNumberGenerator.new()
		rng.randomize()
		if rng.randi_range(1, 100) <= 10:
			print("Pre-generating next room (10% chance triggered)")
			...
			if not next_available_scenes.is_empty():
				...
				if src_conn != null:
					...
					if next_conn != null:
						...
						print("Pre-generated next room attached to: ", src_conn.name, " at position: ", src_conn.global_transform.origin)
						
						# Set the flag in level script
						if level_script:
							level_script.RoomPreGenerated = true
							print("Set RoomPreGenerated flag to true in level script")
					else:
			...
			print("Pre-generation skipped (90% chance)")
